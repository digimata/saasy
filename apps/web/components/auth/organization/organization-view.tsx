...
// import type { OrganizationViewPath } from "../../server" // TODO: not vendored
// import type { AccountViewProps } from "@/components/auth/account/account-view"
// import { ApiKeysCard } from "@/components/auth/settings/api-key/api-keys-card"
...
type OrganizationViewPath = string

export type OrganizationViewPageProps = Omit<any, "view"> & {
    slug?: string
                        ...
                        <DrawerContent>
                            ...
                            <div className="flex flex-col px-4 pb-4">
                                {navItems.map((item) => (
                                    <Link
                                        key={item.view}
                                        href={`${organizationOptions?.basePath}${organizationOptions?.pathMode === "slug" ? `/${slug}` : ""}/${(organizationOptions?.viewPaths as Record<string, string>)?.[item.view]}`}
                                    >
                                        <Button
                                            size="lg"
                                            className={cn(
